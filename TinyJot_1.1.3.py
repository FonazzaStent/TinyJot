"""TinyJot 1.1.3 - An FTP-enabled notepad.
...
def create_menu():
    ...
    sub_menu.add_command(compound="left",label="FTP Password", command=configure,accelerator="Alt+P")
    ...
    top.bind_all("<Alt-p>",configure_hotkey)
...
def ftp_open():
    try:
        ...
        try:
            ...
            tempfile.close()
            ftp.storbinary("STOR " + filename, open(tempfile, 'rb'))
            tempfile.close()
        tempfile.close()
...
def ftp_save():
    try:
        ...
        print (e)
        messagebox.showerror("FTP error", "Could not connect to FTP server. Please check password")
