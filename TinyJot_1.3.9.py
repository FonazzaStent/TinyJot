"""TinyJot 1.3.9 - A WebDAV and encryption enabled notepad.
...
# --- PATCH webdavclient3 to handle missing Content-Length ---
def safe_download_file(self, remote_path, local_path, progress=None, progress_args=()):
    response = self.execute_request('download', remote_path)
...
def webdav_open_file(filepath):
    ...
    webdav.download_sync(remote_path=filepath, local_path=tempfile.name)
    tempfile.close()
...
def aboutbox():
    ...
    url_title.configure(text="TinyJot 1.3.8", font=("Arial",15), anchor='center')
